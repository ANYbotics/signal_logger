...
enum class LoggerType : int { TypeUnknown = -1, TypeNone = 0, TypeStd = 1, TypeRos = 2 };
...
void add(const ValueType_& var, const std::string& name, const std::string& group = LOG_ELEMENT_DEFAULT_GROUP_NAME,
         const std::string& unit = LOG_ELEMENT_DEFAULT_UNIT, const std::size_t divider = LOG_ELEMENT_DEFAULT_DIVIDER,
         const LogElementAction action = LOG_ELEMENT_DEFAULT_ACTION, const std::size_t bufferSize = LOG_ELEMENT_DEFAULT_BUFFER_SIZE,
 ...
 */
template <typename ValueType_>
typename std::enable_if<std::is_base_of<Eigen::MatrixBase<ValueType_>, ValueType_>::value>::type add(
    const ValueType_& var, Eigen::Ref<MatrixXstring> names, const std::string& group = LOG_ELEMENT_DEFAULT_GROUP_NAME,
    const std::string& unit = LOG_ELEMENT_DEFAULT_UNIT, const std::size_t divider = LOG_ELEMENT_DEFAULT_DIVIDER,
    const LogElementAction action = LOG_ELEMENT_DEFAULT_ACTION, const std::size_t bufferSize = LOG_ELEMENT_DEFAULT_BUFFER_SIZE,
    const BufferType bufferType = LOG_ELEMENT_DEFAULT_BUFFER_TYPE) {
  ...
  for (std::size_t i = 0; i < var.size(); ++i) {
    add<typename ValueType_::Scalar>(*static_cast<const typename ValueType_::Scalar* const>(var.data() + i),
                                     static_cast<std::string>(*(names.data() + i)), group, unit, divider, action, bufferSize, bufferType);
  }
}
