...
startNo = 0;       % number of first data file (filename will be generated based on this number
...
for k=1:length(vars)
    disp([num2str(k) ' ' char(vars(k).name)]);
    eval([' global idx_' char(vars(k).name)]);
    eval(['idx_' char(vars(k).name) '=' num2str(k) ';']);
end
