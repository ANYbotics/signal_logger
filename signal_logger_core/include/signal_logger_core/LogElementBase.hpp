...
class LogElementBase: public LogElementInterface
{
                   ...
                   copyMutex_()
 {
  ...
  virtual void saveDataToLogFile(const std::vector<TimestampPair, Eigen::aligned_allocator<TimestampPair>> & times, unsigned int nrCollectDataCalls, LogFileType type = LogFileType::BINARY) override {}
   ...
   */
  template<typename V = ValueType_>
  const std::vector<V, Eigen::aligned_allocator<V>> & getTimeBufferCopy(typename std::enable_if<std::is_same<TimestampPair, V>::value>::type* = 0 /* is timestamp pair */) const
  {
 ...
 protected:
  ...
  //! Local copy
  std::vector<ValueType_, Eigen::aligned_allocator<ValueType_>> bufferCopy_;
  //! Local name copy
